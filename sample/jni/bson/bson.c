...
MONGO_EXPORT bson *bson_empty( bson *obj ) {
    static char *data = (char*)std::string( "\005\0\0\0\0" ).c_str(); //"\005\0\0\0\0";
...
MONGO_EXPORT bson_type bson_iterator_next( bson_iterator *i ) {
    ...
    i->progress += 1 + strlen( i->cur + 1 ) + 1 + ds;
...
char *bson_realloc( char *ptr, size_t size ) {
    char *p;
    p = (char*)bson_realloc_func( ptr, size );
